...
//! - 记录 token 消耗并在超预算时截断旧消息

use crate::model::{Message, ModelRequest, TokenUsage, ToolSpec};
...
/// 无状态会话管理器。
///
/// 持有 system prompt 和对话历史，负责拼装 `ModelRequest`。
...
/// ```
pub struct Session {
    /// 固定的 system prompt，每次请求作为第一条 Message::System 发送。
    system_prompt: String,
    /// 对话历史（不含 system prompt）。
    messages: Vec<Message>,
    /// 累计消耗的 token 总数（从 API `usage` 字段获取）。
    total_tokens: u32,
    /// Token 预算上限。超过时 `truncate_to_budget()` 会删除最旧的消息。
    ///
...
impl Session {
    ...
    /// 追加一条消息到对话历史。
    ///
    /// Agent 循环中每收到一条消息就调用：User 输入、Assistant 回复、Tool 结果。
    /// 参考 Claude Code `query.ts`: `state.messages.push(response.message)`。
    ...
    ///
    /// 组装顺序：`[Message::System(system_prompt), ...self.messages, tools]`
    ///
    /// 参考 Claude Code `claude.ts` 的请求拼装：
    /// system prompt 独立于 messages，通过请求体顶层 `system` 字段发送。
    /// funcode 中统一为 `Message::System` 放在 messages[0]，
    /// `model.rs` 的 `message_to_openai()` 会将其转为对应 provider 的格式。
    pub fn build_request(&self, tools: &[ToolSpec]) -> ModelRequest {
        let mut all = Vec::with_capacity(self.messages.len() + 1);
        all.push(Message::system(&self.system_prompt));
    ...
    pub fn record_usage(&mut self, usage: TokenUsage) {
        if let Some(total) = usage.total_tokens {
            ...
            let sum = usage.input_tokens.unwrap_or(0)
    ...
    /// 按 token 预算截断旧消息。
    ///
    /// 使用 `len() / 4` 启发式估算每条消息的 token 数（参考 Codex CLI
    /// `approx_token_count`），从最旧的消息开始删除，直到总量低于预算。
    ///
    /// 保留最近一条 User 消息不截断，确保 agent 始终知道用户在问什么。
    ...
    fn estimate_tokens(&self) -> usize {
        self.messages
            .iter()
            .map(|m| {
                let text = m.text_content().map(|t| t.len()).unwrap_or(0);
...
mod tests {
    use super::*;
    use crate::model::{AssistantBlock, ToolCall, ToolResult};
    use serde_json::json;
    ...
    fn push_and_read_messages() {
        ...
        session.push(Message::assistant_text("hi"));

        assert_eq!(session.len(), 2);
        assert!(matches!(session.messages()[0], Message::User(ref s) if s == "hello"));
    ...
    fn build_request_includes_system_prompt() {
        ...
        assert!(matches!(
            &req.messages[0],
            Message::System(s) if s == "You are a Rust expert."
        ));
    }
    ...
    fn build_request_prepends_system_to_history() {
        ...
        assert!(matches!(&req.messages[0], Message::System(_)));
        assert!(matches!(&req.messages[1], Message::User(_)));
        assert!(matches!(&req.messages[2], Message::Assistant(_)));
    }
    ...
    fn build_request_does_not_mutate_session() {
        ...
        // build_request 不应改变 messages
        assert_eq!(session.len(), 1);
        // messages 里只有 User，不含 System（System 在 build_request 时拼装）
    ...
    #[test]
    fn truncate_removes_oldest_messages() {
        let mut session = Session::new("system", 10); // 极小预算
        // 每条消息约 3 token ("aaa" = 3 bytes / 4 ≈ 1 token)
        // 塞入 20 条，远超预算
        for i in 0..20 {
            session.push(Message::user(format!("aaa{i}")));
        }
    ...
    #[test]
    fn truncate_keeps_last_user_message() {
        let mut session = Session::new("system", 10);
        for i in 0..20 {
            session.push(Message::user(format!("msg{i}")));
        }

        let last_text = session.messages().last().and_then(|m| m.text_content()).unwrap().to_string();
        session.truncate_to_budget();

        // 最后一条 User 消息必须保留
        let final_text = session.messages().last().and_then(|m| m.text_content()).unwrap();
    ...
    fn truncate_noop_when_within_budget() {
        ...
        session.push(Message::assistant_text("ok"));
    ...
    fn clear_resets_history_but_keeps_prompt() {
        ...
        assert!(matches!(
            &req.messages[0],
            Message::System(s) if s == "system prompt"
        ));
    }
    ...
    fn simulate_agent_turn_with_tool_call() {
        ...
        // 1. 用户输入
        session.push(Message::user("Read src/main.rs"));

        // 2. 模型返回 assistant + tool_call
        let req1 = session.build_request(&tools);
        assert_eq!(req1.messages[0].text_content(), Some("You are helpful."));
        assert_eq!(req1.messages[1].text_content(), Some("Read src/main.rs"));

        session.push(Message::assistant(vec![
            AssistantBlock::text("Let me read that file."),
        ...
        session.push(Message::tool(call, result));

        // 4. 构建下一轮请求（此时还没有 push 最终回复）
        let req2 = session.build_request(&tools);
        assert_eq!(req2.messages.len(), 4); // system + user + assistant + tool

        session.push(Message::assistant_text("The file contains `fn main() {}`."));
        session.record_usage(TokenUsage {
        ...
        // 验证最终状态
        assert_eq!(session.len(), 4); // user + assistant + tool + assistant
